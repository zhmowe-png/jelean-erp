...
export function DeliveryDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          setNote(nRes.data as unknown as DeliveryNote);
          setItems(iRes.data || []);
        }
      } catch (e) {
  ...
  const customer = (note as unknown as Record<string, unknown>).customers as Record<string, string> | undefined || {};
      ...
      <Printable>
        ...
        <div className="bg-white rounded-lg border p-6 print:border-0 print:p-0">
            ...
            <tbody>
              {items.map((it) => (
                <tr key={it.id} className="print:border-b">
                  ...
                  <td className="px-3 py-1.5 border text-right">
                    {it.quantity}
                  </td>
                  <td className="px-3 py-1.5 border text-right">
                    {Number(it.unit_price).toFixed(4)}
                  </td>
                  <td className="px-3 py-1.5 border text-right font-mono">
              ...
              {/* Empty rows for print (fill rest of page) */}
          ...
          <div className="hidden print:flex justify-between mt-10 text-sm">
            <div>
              收货单位及经手人：{note.receiver || "____________"}
            </div>
          ...
          <div className="print:hidden mt-3 text-sm">
            <span className="text-gray-500">收货单位及经手人：</span>
            {note.receiver || "（未填写）"}
          </div>
        </div>
      </Printable>
