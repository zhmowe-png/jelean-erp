...
export function DeliveryForm() {
  ...
  function mkItem(seq: number): LineItem {
    return { key: Date.now() + seq, seq, order_number: "", material_code: "", product_name: "", quantity: "", unit_price: "", amount: 0, notes: "" };
  }
  ...
  function addRow() {
    setItems((prev) => [...prev, mkItem(prev.length + 1)]);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (filled.length === 0) { setError("请至少填写一条送货明细"); return; }
    for (const it of filled) {
      if (!it.quantity || parseFloat(it.quantity) <= 0) { setError(`第${it.seq}行数量不合法`); return; }
    }
    ...
    if (noteErr) {
      setError("创建送货单失败: " + noteErr.message);
      setSaving(false);
      return;
    }
    ...
    if (itemsErr) {
      setError("保存明细失败: " + itemsErr.message);
      setSaving(false);
      return;
    }
        ...
        <div className="bg-white rounded-lg border p-5 mb-4">
          <div className="grid grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input className={inputCls} value="系统自动生成" disabled />
            </div>
            <div>
        ...
        <div className="bg-white rounded-lg border overflow-hidden mb-4">
              ...
              {items.map((it) => (
                <tr key={it.key} className="border-t">
                  ...
                  <td className="px-3 py-1.5">
                    <input className={inputCls} value={it.order_number} onChange={(e) => updateItem(it.key, "order_number", e.target.value)} />
                  </td>
                  <td className="px-3 py-1.5">
                    <input className={inputCls} value={it.material_code} onChange={(e) => updateItem(it.key, "material_code", e.target.value)} />
                  </td>
                  <td className="px-3 py-1.5">
                    <input className={inputCls} value={it.product_name} onChange={(e) => updateItem(it.key, "product_name", e.target.value)} />
                  </td>
                  <td className="px-3 py-1.5">
                    <input type="number" step="1" className={`${inputCls} text-right`} value={it.quantity} onChange={(e) => updateItem(it.key, "quantity", e.target.value)} />
                  </td>
                  <td className="px-3 py-1.5">
                    <input type="number" step="0.0001" className={`${inputCls} text-right`} value={it.unit_price} onChange={(e) => updateItem(it.key, "unit_price", e.target.value)} />
                  </td>
                  ...
                  <td className="px-3 py-1.5 text-center">
                    {items.length > 1 && (
                      <button type="button" onClick={() => removeRow(it.key)} className="text-red-400 hover:text-red-600 text-lg leading-none">&times;</button>
                    )}
                  </td>
                </tr>
              ))}
          ...
          <div className="px-4 py-2 border-t bg-gray-50 flex justify-between items-center">
            ...
            <div className="text-sm">
              合计：<span className="font-bold text-lg">¥{totalAmount.toLocaleString("zh-CN", { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
        </div>
